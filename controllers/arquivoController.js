  ...
  const verifyAndUpdateUser = async (req, res, next) => {
    try {
      const { mtid, cid, nome_usuario } = req.query;
      if (!mtid || !cid || !nome_usuario) {
        return res.status(400).send({ message: 'Dados incompletos para autenticação' });
      }
  
      const data = await fs.readFile(USERS_FILE, 'utf8');
      const users = JSON.parse(data);
      const user = users.find(u => u.mtid === mtid);
      ...
      if (user) {
        ...
        const updatedUsers = users.map(u => u.mtid === mtid ? updatedUser : u);
